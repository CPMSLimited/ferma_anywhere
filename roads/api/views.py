...
from roads.api.serializers import (
    SegmentSerializer, 
    AddressSerializer, 
    RouteSerializer, 
    SegmentUploadItemSerializer # use item serializer with many=True
    )
...
from .utils.google_maps import fetch_distance_matrix_pairwise

BATCH_SIZE = 25  # tune for Distance Matrix limits
TIMEOUT = 8

def _status_from_speed(kmh: float) -> str:
    if kmh < 1:   return '666699'
    if kmh < 40:  return 'FF0000'
    if kmh < 50:  return 'FF5050'
    if kmh < 60:  return 'FF9966'
    if kmh < 70:  return 'FFFFCC'
    if kmh < 80:  return '00CC00'
    if kmh < 90:  return '339933'
    return '006600'

@api_view(["POST"])
def gpt_bulk_segments_upload(request):
    # Validate & normalise input
    ser = SegmentUploadItemSerializer(data=request.data, many=True)
    ser.is_valid(raise_exception=True)
    rows = [item["_normalised"] for item in ser.validated_data]

    # … do your DB + Google work here (service or inline) …
    # If you’re still stubbing:
    return Response({"count": len(rows)}, status=status.HTTP_200_OK)
...
def bulk_segments_upload(request):
    if request.method == 'POST':
        ...
        for obj in request.data:
            routes.append(Route(route=obj.get("ROUTE"), route_index=obj.get("INDEX")))
        ...
        for obj in request.data:
            ...
            if json.dumps(obj.get("SEGMENT_NAME")) != 'null' and json.dumps(obj.get("STATE")) != 'null':
                ...
                # except:
                #     return Response(
                #         {'message': 'likely error assigning route to segment' ,
                #         'error': 'roads.models.Route.DoesNotExist: Route matching query does not exist.'}, 
    ...
    for point in addresses:
        start_lat = point.get('start_lat')
        start_lng = point.get('start_lng')
        end_lat = point.get('end_lat')
        end_lng = point.get('end_lng')

        # Skip invalid coordinates
        if not all([start_lat, start_lng, end_lat, end_lng]):
            continue  # or log this as an error and skip

        # Optional: validate that lat/lng are within bounds
        try:
            slat = float(start_lat.strip('"'))
            slng = float(start_lng.strip('"'))
            elat = float(end_lat.strip('"'))
            elng = float(end_lng.strip('"'))

            if not (-90 <= slat <= 90 and -180 <= slng <= 180 and -90 <= elat <= 90 and -180 <= elng <= 180):
    ...
    Address.objects.bulk_create(google_addresses, ignore_conflicts=True) # raise exceptions.ValidationError(django.core.exceptions.ValidationError: ['“null” value must be a decimal number.']

    segments_to_update = []
    for segment in segments:
        print("here 1", segment)
        ...
        try:
            ...
            seg.index = segment['code'][-1:] # TypeError: 'NoneType' object is not subscriptable
        # if last_two.isalpha():
        #     print(seg.index)
        # else:
